...
class AsyncTelegramController:
    ...
    async def start_session(self):
        if self.client_session:
            await self.client_session.start()
            ...
            if message['chat']['type'] == 'private':
                try:
                    ...
                    if key == 'new':
                        ...
                        client_session.send_message(sender_id, msg)
                    else:
                        ...
                        if response.status_code == 204:
                            client_session.send_message(sender_id, 'status:object not found')
                        elif response.status_code == 200:
                            msg = response.json()['text']
                            client_session.send_message(sender_id, msg)
                        else:
                            client_session.send_message(sender_id, 'status:bad request')
                except:
                    pass
